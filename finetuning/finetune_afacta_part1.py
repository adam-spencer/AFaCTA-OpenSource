...
TODO:
=====
  - Change args to allow selection of params
  - Change params for HPC
  - Save model
  - Save training loss
  - Save evaluation performance - accuracy is not enough!
"""
...
def load_prompt_data(data_path):
    ...
    val_set = train_val_split['validation']
    return train_set, val_set


def formatting_prompts_func(input_line, tokenizer):
    # The tokenizer requires a list of dictionaries
    text = tokenizer.apply_chat_template(
        input_line['messages'], tokenize=False, add_generation_prompt=False)
    return {'text': text}


def train(model, tokenizer, train_dataset, validation_dataset):
    trainer = SFTTrainer(
        model=model,
        tokenizer=tokenizer,
...
def evaluation(model, tokenizer, validation_set):
...
if __name__ == '__main__':
    ...
    parser.add_argument('finetuning_data', help='Path to data for finetuning')
    args = parser.parse_args()
    model, tokenizer = load_model(args.model_name)
    train_set, val_set = load_prompt_data(args.finetuning_data)
    train(model, tokenizer, train_set, val_set)
    evaluation(model, tokenizer, val_set)
    # TODO save model
